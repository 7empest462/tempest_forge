...
impl WallConstructor {
    pub fn from_curve(curve: &Curve, wall_height: f32, get_ground_y: impl Fn(Vec3) -> f32) -> Vec<Brick> {
        ...
        for r in 0..max_row_count {
            ...
            if is_odd {
                ...
                split_points = (0..=bricks_per_row).map(|k| k as f32 / bricks_per_row as f32).collect();
            }

            // Perturb splits slightly for organic variation (running bond with hand-built look!)
            let brick_widths = perturb_splits(&split_points, BRICK_WIDTH_VARIANCE / wall_length, &mut rng);
            ...
            for j in 0..brick_widths.len() {
                if let Some(&next_u) = brick_widths.get(j+1) {
                    ...
                    if rng.f32() < 0.4 && r != 0 {
                        ...
                        for (height, pivot_v) in vec![(height_u_1, pivot_v_1), (height_u_2, pivot_v_2)] {
