...
fn update_wall_builder(
    ...
    if mouse_input.just_pressed(MouseButton::Right) || gamepad_place {
        if let Some(pt) = hover_point {
            ...
            for _ in 0..4 {
                commands.spawn((
                    Mesh3d(meshes.add(Cuboid::from_size(Vec3::splat(0.08)))),
                    ...
                    crate::player::interaction::Particle {
                        velocity: Vec3::new(rng.random_range(-1.5..1.5), rng.random_range(1.5..3.5), rng.random_range(-1.5..1.5)),
            ...
            for (idx, brick) in bricks.iter().enumerate() {
                ...
                commands.spawn((
                    ...
                    bevy_rapier3d::prelude::Collider::cuboid(
                        brick.transform.scale.x / 2.0,
                        brick.transform.scale.y / 2.0,
                        brick.transform.scale.z / 2.0,
                    ),
                    BrickSpawnAnimation {
                    ...
                    let has_left_neighbor = bricks.iter().enumerate().any(|(i, other)| {
                        i != idx 
                            ...
                            && (brick.pivot_uv.x - brick.bounds_uv.x / 2.0 - (other.pivot_uv.x + other.bounds_uv.x / 2.0)).abs() < 0.02
                    });
                    let has_right_neighbor = bricks.iter().enumerate().any(|(i, other)| {
                        i != idx 
                            && (other.pivot_uv.y - brick.pivot_uv.y).abs() < 0.01 
                            && (other.pivot_uv.x - other.bounds_uv.x / 2.0 - (brick.pivot_uv.x + brick.bounds_uv.x / 2.0)).abs() < 0.02
                    });
                    let has_top_neighbor = bricks.iter().enumerate().any(|(i, other)| {
                        i != idx 
                            && other.pivot_uv.y > brick.pivot_uv.y
                            && (other.pivot_uv.y - other.bounds_uv.y / 2.0 - (brick.pivot_uv.y + brick.bounds_uv.y / 2.0)).abs() < 0.02
                            && (other.pivot_uv.x - brick.pivot_uv.x).abs() < (brick.bounds_uv.x + other.bounds_uv.x) / 2.0 - 0.01
                    });
                    let has_bottom_neighbor = bricks.iter().enumerate().any(|(i, other)| {
                        i != idx 
                            && other.pivot_uv.y < brick.pivot_uv.y
                            && (brick.pivot_uv.y - brick.bounds_uv.y / 2.0 - (other.pivot_uv.y + other.bounds_uv.y / 2.0)).abs() < 0.02
                            && (other.pivot_uv.x - brick.pivot_uv.x).abs() < (brick.bounds_uv.x + other.bounds_uv.x) / 2.0 - 0.01
                    });
                    ...
                    let rel_x = ((brick.transform.scale.x - (left_shrink + right_shrink)) / brick.transform.scale.x).max(0.1);
                    let rel_y = ((brick.transform.scale.y - (bottom_shrink + top_shrink)) / brick.transform.scale.y).max(0.1);
                    ...
                    let mortar_rel_x = 1.02 - (mortar_left_inset + mortar_right_inset) / brick.transform.scale.x;
                    let mortar_rel_y = 1.02 - (mortar_top_inset + mortar_bottom_inset) / brick.transform.scale.y;

                    // Mortar offsets also in parent-local space
                    let mortar_trans_x = (mortar_left_inset - mortar_right_inset) / (2.0 * brick.transform.scale.x);
                    let mortar_trans_y = (mortar_bottom_inset - mortar_top_inset) / (2.0 * brick.transform.scale.y);
                ...
                if rng.f32() < 0.25 {
                    commands.spawn((
                        Mesh3d(meshes.add(Cuboid::from_size(Vec3::splat(0.12)))),
                        MeshMaterial3d(materials.add(StandardMaterial {
                            base_color: Color::srgba(0.8, 0.75, 0.7, 0.55),
...
fn draw_wall_preview(
    ...
    // holographic translucent brick layout projection
    if builder.points.len() >= 2 {
        let raw_curve = Curve::from(builder.points.clone()).smooth(2);
        let resampled_curve = raw_curve.resample(0.8);
        let bricks = WallConstructor::from_curve(&resampled_curve, builder.height, |pos| {
...
fn mine_procedural_bricks(
    ...
    mut brick_query: Query<(Entity, &GlobalTransform, &mut crate::player::combat::Health, &Transform), With<ProceduralBrick>>,
    ...
    if !mouse_input.pressed(MouseButton::Left) || *weapon != crate::player::combat::WeaponState::Pickaxe {
        return;
    }
    ...
    if let Some((entity, mut health, pos, scale)) = closest_brick {
        ...
        if rng.random_bool(0.18) {
            let p_pos = pos + Vec3::new(rng.random_range(-0.1..0.1), rng.random_range(-0.1..0.1), rng.random_range(-0.1..0.1));
            commands.spawn((
                Mesh3d(meshes.add(Cuboid::from_size(Vec3::splat(0.04)))),
                MeshMaterial3d(materials.add(StandardMaterial {
                ...
                crate::player::interaction::Particle {
                    velocity: Vec3::new(rng.random_range(-1.2..1.2), rng.random_range(1.5..3.5), rng.random_range(-1.2..1.2)),
        ...
        if health.hp <= 0.0 {
            ...
            *inventory.resources.entry(BlockType::Brick).or_insert(0) += 1;
            println!("Collected: Brick (Total: {})", inventory.resources[&BlockType::Brick]);

            // Splinter particles
            for _ in 0..5 {
                let p_pos = pos + Vec3::new(rng.random_range(-scale.x/2.0..scale.x/2.0), rng.random_range(-scale.y/2.0..scale.y/2.0), rng.random_range(-scale.z/2.0..scale.z/2.0));
                commands.spawn((
                    ...
                    Transform::from_translation(p_pos),
                    crate::player::interaction::Particle {
                        velocity: Vec3::new(rng.random_range(-2.0..2.0), rng.random_range(1.5..4.0), rng.random_range(-2.0..2.0)),
                        lifetime: Timer::from_seconds(0.8 + rng.random_range(0.0..0.4), TimerMode::Once),
                    },
                ));
            }
        }
        ...
        if progress >= 1.0 {
            ...
            for _ in 0..2 {
                commands.spawn((
                    ...
                    crate::player::interaction::Particle {
                        velocity: Vec3::new(rng.random_range(-1.0..1.0), rng.random_range(0.2..1.2), rng.random_range(-1.0..1.0)),
...
fn carve_gateways(
    ...
    if let Some((_entity, carve_pos, carve_rot)) = targeted_brick {
        ...
        for (_, global_transform, transform) in brick_query.iter() {
            let pos = global_transform.translation();
            let horizontal_dist = Vec2::new(pos.x, pos.z).distance(Vec2::new(carve_pos.x, carve_pos.z));
            ...
            if horizontal_dist < 1.2 && pos.y <= top_y + 0.1 {
                ...
                for _ in 0..2 {
                    commands.spawn((
                        Mesh3d(meshes.add(Cuboid::from_size(Vec3::splat(0.1)))),
                        MeshMaterial3d(materials.add(StandardMaterial {
                            base_color: Color::srgba(0.65, 0.62, 0.6, 0.8),
                            ..default()
                        })),
        ...
        commands.spawn((
            ...
            gate.spawn((
                crate::player::interaction::Door { open: false, hinge_side: -1.0 },
                Transform::from_xyz(-1.2, 0.0, 0.0),
                Visibility::default(),
                InheritedVisibility::default(),
            )).with_children(|hinge| {
                hinge.spawn((
                    Mesh3d(meshes.add(Cuboid::new(1.2, door_height, 0.12))),
                    MeshMaterial3d(materials.add(StandardMaterial {
                        base_color: Color::srgb(0.38, 0.22, 0.12), // Medieval dark oak wood
                        perceptual_roughness: 0.85,
                        ..default()
                    })),
                    Transform::from_xyz(0.6, door_height / 2.0, 0.0), // Center door panel between hinge and opening center
