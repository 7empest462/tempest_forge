    ...
    for (entity, chunk_comp) in query.iter() {
        ...
        if let Ok(mut chunk_cmd) = commands.get_entity(entity) {
            ...
            if is_settlement_chunk && (chunk_pos.x != 0 || chunk_pos.z != 0) {
                ...
                let h = find_stable_ground_height(Vec3::new(town_base_x, (chunk_pos.y * 16 + 16) as f32, town_base_z), &voxel_world);
                if let Some(h) = h {
                    ...
                    let (tavern_pos, shop_pos, forge_pos, farm_pos, house1_pos, house2_pos, tower_pos, town_waypoints) = {
                        ...
                        let node1 = tavern_pos.map(|p| p + Vec3::new(0.0, 0.0, -6.75)).unwrap_or(node0);
                        let node2 = shop_pos.map(|p| p + Vec3::new(-4.5, 0.0, 0.0)).unwrap_or(node0);
                        let node3 = forge_pos.map(|p| p + Vec3::new(4.5, 0.0, 0.0)).unwrap_or(node0);
                        let node4 = farm_pos.map(|p| p + Vec3::new(0.0, 0.0, 4.5)).unwrap_or(node0);
                        let node5 = house1_pos.map(|p| p + Vec3::new(0.0, 0.0, 4.5)).unwrap_or(node0);
                        let node6 = house2_pos.map(|p| p + Vec3::new(0.0, 0.0, 4.5)).unwrap_or(node0);
                        let node7 = tower_pos.map(|p| p + Vec3::new(-3.0, 0.0, 3.0)).unwrap_or(node0);
                        let node8 = Vec3::new(town_base_x, get_surface(town_base_x, town_base_z + 32.0).unwrap_or(h), town_base_z + 32.0);
                        let node9 = Vec3::new(town_base_x, get_surface(town_base_x, town_base_z - 32.0).unwrap_or(h), town_base_z - 32.0);

                        let town_waypoints = vec![node0, node1, node2, node3, node4, node5, node6, node7, node8, node9];
                        (tavern_pos, shop_pos, forge_pos, farm_pos, house1_pos, house2_pos, tower_pos, town_waypoints)
                    };
                    ...
                    for dx in -7..=7 {
                        for dz in -7..=7 {
                            ...
                            let start_y = if let Some(sy) = find_stable_ground_height(Vec3::new(vx as f32, (chunk_pos.y * 16 + 16) as f32, vz as f32), &voxel_world) {
                            ...
                            for vy in start_y..=target_y {
                                ...
                                voxel_world.set_voxel(pos, WorldVoxel::Solid(BlockType::Cobblestone as u8));
                            }
                        }
                    }
                    ...
                    commands.spawn((
                        ...
                        SettlementWaypoints { nodes: town_waypoints.clone() },
                        SettlementBuilding { building_type: BuildingType::Plaza, center: plaza_center },
                    )).with_children(|town| {
                        // 1. Central Plaza Pavement
                        town.spawn((
                            Mesh3d(meshes.add(Cuboid::new(15.0, 0.05, 15.0))),
                            MeshMaterial3d(materials.add(StandardMaterial { base_color: Color::srgb(0.3, 0.3, 0.32), perceptual_roughness: 0.9, ..default() })),
                        ...
                        town.spawn((
                            ...
                            MeshMaterial3d(materials.add(StandardMaterial { base_color: Color::srgb(0.25, 0.25, 0.25), ..default() })),
                            ...
                            well.spawn((
                                ...
                                MeshMaterial3d(materials.add(StandardMaterial { base_color: Color::srgb(0.1, 0.3, 0.7), perceptual_roughness: 0.1, metallic: 0.5, ..default() })),
                            ...
                            for side in [-1.0, 1.0] {
                                well.spawn((
                                    ...
                                    MeshMaterial3d(materials.add(StandardMaterial { base_color: Color::srgb(0.2, 0.12, 0.08), ..default() })),
                            ...
                            well.spawn((
                                ...
                                MeshMaterial3d(materials.add(StandardMaterial { base_color: Color::srgb(0.4, 0.15, 0.1), ..default() })),
                                Transform::from_xyz(0.0, 2.4, 0.0).with_rotation(Quat::from_rotation_x(0.2)),
                            ));
                        });

                        // 3. Sprawling gravel paths connecting building doors back to plaza node0
                        if tavern_pos.is_some() { spawn_road_path(town, &mut meshes, &mut materials, town_waypoints[0], town_waypoints[1], plaza_center, &voxel_world); }
                        if shop_pos.is_some() { spawn_road_path(town, &mut meshes, &mut materials, town_waypoints[0], town_waypoints[2], plaza_center, &voxel_world); }
                        if forge_pos.is_some() { spawn_road_path(town, &mut meshes, &mut materials, town_waypoints[0], town_waypoints[3], plaza_center, &voxel_world); }
                        if farm_pos.is_some() { spawn_road_path(town, &mut meshes, &mut materials, town_waypoints[0], town_waypoints[4], plaza_center, &voxel_world); }
                        if house1_pos.is_some() { spawn_road_path(town, &mut meshes, &mut materials, town_waypoints[0], town_waypoints[5], plaza_center, &voxel_world); }
                        if house2_pos.is_some() { spawn_road_path(town, &mut meshes, &mut materials, town_waypoints[0], town_waypoints[6], plaza_center, &voxel_world); }
                        if tower_pos.is_some() { spawn_road_path(town, &mut meshes, &mut materials, town_waypoints[0], town_waypoints[7], plaza_center, &voxel_world); }

                        // 4. Street lamp posts around the plaza
                        let lamp_positions = [
                            Vec3::new(6.75, 0.0, 6.75),
                            Vec3::new(-6.75, 0.0, -6.75),
                        ];
                        for pos in lamp_positions {
                            town.spawn((
                                Mesh3d(meshes.add(Cuboid::new(0.24, 4.2, 0.24))),
                                MeshMaterial3d(materials.add(StandardMaterial { base_color: Color::srgb(0.15, 0.15, 0.15), ..default() })),
                    ...
                    if let Some(pos) = tavern_pos {
                        spawn_building(&mut commands, &mut meshes, &mut materials, pos, BuildingType::Tavern, smoke_handle.clone());
                        spawn_npc(&mut commands, &mut meshes, &mut materials, pos + Vec3::new(-1.5, 0.5, -1.0), NPCRole::Barkeeper, Some(pos), town_waypoints.clone());
                    }

                    if let Some(pos) = shop_pos {
                        spawn_building(&mut commands, &mut meshes, &mut materials, pos, BuildingType::Shop, smoke_handle.clone());
                        spawn_npc(&mut commands, &mut meshes, &mut materials, pos + Vec3::new(0.0, 0.8, 0.0), NPCRole::Merchant, Some(pos), town_waypoints.clone());
                    }

                    if let Some(pos) = forge_pos {
                        spawn_building(&mut commands, &mut meshes, &mut materials, pos, BuildingType::Forge, smoke_handle.clone());
                        spawn_npc(&mut commands, &mut meshes, &mut materials, pos + Vec3::new(0.0, 0.8, 1.0), NPCRole::Blacksmith, Some(pos), town_waypoints.clone());
                    }

                    if let Some(pos) = farm_pos {
                        spawn_building(&mut commands, &mut meshes, &mut materials, pos, BuildingType::House, smoke_handle.clone());
                        spawn_npc(&mut commands, &mut meshes, &mut materials, pos + Vec3::new(-1.2, 0.5, -0.8), NPCRole::Farmer, Some(pos), town_waypoints.clone());

                        // Spawn Fenced Wheat Crop Garden next to farm
                        spawn_garden_field(&mut commands, &mut meshes, &mut materials, pos + Vec3::new(5.5, 0.0, 0.0));
                    }

                    if let Some(pos) = house1_pos {
                        spawn_building(&mut commands, &mut meshes, &mut materials, pos, BuildingType::House, smoke_handle.clone());
                        spawn_npc(&mut commands, &mut meshes, &mut materials, pos + Vec3::new(-1.2, 0.5, -0.8), NPCRole::Citizen, None, town_waypoints.clone());
                    }
                    ...
                    if let Some(pos) = tower_pos {
                        spawn_building(&mut commands, &mut meshes, &mut materials, pos, BuildingType::GuardTower, None);
                        
                        // Patrol Guards
                        spawn_npc(&mut commands, &mut meshes, &mut materials, pos + Vec3::new(-1.0, 1.0, 1.0), NPCRole::Guard, None, town_waypoints.clone());
                        spawn_npc(&mut commands, &mut meshes, &mut materials, plaza_center + Vec3::new(4.0, 1.0, -4.0), NPCRole::Guard, None, town_waypoints.clone());
                    }
                }
            }
        }
...
fn spawn_road_path(
    ...
    for i in 0..num_segments {
        ...
        let rot = Quat::from_rotation_y(-yaw + std::f32::consts::FRAC_PI_2) * Quat::from_rotation_x(pitch);
        ...
        if !is_bridge_seg {
            ...
            town.spawn((
                ...
                Transform::from_translation(seg_mid + Vec3::new(0.0, 0.015, 0.0)).with_rotation(rot),
                ...
                for side in [-1.0, 1.0] {
                    for z_offset in [-1.0, 1.0] {
                        deck.spawn((
                            ...
                            MeshMaterial3d(bridge_rail_mat.clone()),
                            Transform::from_xyz(side * 1.05, 0.225, z_offset * (seg_dist / 2.0 - 0.05)),
                        ));
                    }
                }
...
fn spawn_garden_field(
    ...
    let dirt_mat = materials.add(StandardMaterial { base_color: Color::srgb(0.18, 0.12, 0.08), perceptual_roughness: 0.95, ..default() });
    let wood_mat = materials.add(StandardMaterial { base_color: Color::srgb(0.3, 0.18, 0.1), ..default() });
    let crop_stem_mat = materials.add(StandardMaterial { base_color: Color::srgb(0.2, 0.5, 0.1), ..default() });
    let crop_gold_mat = materials.add(StandardMaterial { base_color: Color::srgb(0.8, 0.6, 0.1), ..default() });
...
fn spawn_building(
    ...
    let main_mat = materials.add(StandardMaterial { base_color: Color::srgb(0.85, 0.80, 0.72), perceptual_roughness: 0.9, ..default() }); // Plaster
    let foundation_mat = materials.add(StandardMaterial { base_color: Color::srgb(0.28, 0.28, 0.28), perceptual_roughness: 0.95, ..default() }); // Cobblestone
    let framing_mat = materials.add(StandardMaterial { base_color: Color::srgb(0.22, 0.14, 0.08), perceptual_roughness: 0.9, ..default() }); // Dark Timber
    let roof_mat = materials.add(StandardMaterial { base_color: Color::srgb(0.48, 0.12, 0.12), perceptual_roughness: 0.8, ..default() }); // Terracotta Red Tiles
    let floor_mat = materials.add(StandardMaterial { base_color: Color::srgb(0.35, 0.22, 0.12), perceptual_roughness: 0.85, ..default() }); // Wood floor
    ...
    commands.spawn((
        ...
        Visibility::default(),
        InheritedVisibility::default(),
        SettlementBuilding { building_type, center: world_pos },
        ...
        if building_type == BuildingType::GuardTower {
            ...
            for dx in [-1.0, 1.0] {
                for dz in [-1.0, 1.0] {
                    building.spawn((
                        ...
                        Transform::from_xyz(dx * (size.x/2.0 + 0.1), size.y + 1.1, dz * (size.z/2.0 + 0.1)),
                    ));
                }
            }
            ...
            building.spawn((
                ...
                Transform::from_xyz(0.0, size.y + 2.4, 0.0).with_scale(Vec3::new(size.x + 1.0, 1.2, size.z + 1.0)),
            ));
        ...
        for dx in [-1.0, 1.0] {
            for dz in [-1.0, 1.0] {
                building.spawn((
                    ...
                    MeshMaterial3d(framing_mat.clone()),
                    Transform::from_xyz(dx * (size.x/2.0 - 0.15), col_h / 2.0, dz * (size.z/2.0 - 0.15)),
                ));
            }
        }
        ...
        for side in [-1.0, 1.0] {
            ...
            building.spawn((
                ...
                MeshMaterial3d(main_mat.clone()),
                Transform::from_xyz(side * (size.x/2.0 - side_panel_w/2.0 - 0.3), col_h / 2.0, size.z/2.0 - wall_t/2.0),
        ...
        for side in [-1.0, 1.0] {
            building.spawn((
                ...
                MeshMaterial3d(framing_mat.clone()),
                Transform::from_xyz(0.0, col_h + (size.x * 0.14) + 0.08, side * (size.z/2.0 - 0.15)),
            ));
        }
        ...
        building.spawn((
            ...
            MeshMaterial3d(materials.add(StandardMaterial { base_color: Color::srgb(0.3, 0.3, 0.3), perceptual_roughness: 0.95, ..default() })),
        ...
        match building_type {
            BuildingType::House | BuildingType::Tavern => {
                ...
                building.spawn((
                    ...
                    MeshMaterial3d(materials.add(StandardMaterial { base_color: Color::srgb(0.72, 0.15, 0.15), perceptual_roughness: 0.8, ..default() })),
        ...
        match building_type {
            BuildingType::Tavern => {
                ...
                building.spawn((
                    ...
                    MeshMaterial3d(materials.add(StandardMaterial { base_color: Color::srgb(0.28, 0.16, 0.1), perceptual_roughness: 0.85, ..default() })),
                    Transform::from_xyz(1.5, 0.475, 1.2),
                    Collider::cuboid(2.0, 0.475, 0.3),
                ));

                // Benches and tables
                let table_mat = materials.add(StandardMaterial { base_color: Color::srgb(0.36, 0.22, 0.14), perceptual_roughness: 0.9, ..default() });
                for side in [-1.0, 1.0] {
                    // Tables
                    building.spawn((
            ...
            BuildingType::Forge => {
                ...
                building.spawn((
                    ...
                    MeshMaterial3d(materials.add(StandardMaterial { base_color: Color::srgb(0.22, 0.22, 0.22), perceptual_roughness: 0.95, ..default() })),
                ...
                building.spawn((
                    ...
                    MeshMaterial3d(materials.add(StandardMaterial { base_color: Color::srgb(0.42, 0.26, 0.15), perceptual_roughness: 0.9, ..default() })),
                    ...
                    base.spawn((
                        ...
                        MeshMaterial3d(materials.add(StandardMaterial { base_color: Color::srgb(0.1, 0.1, 0.12), perceptual_roughness: 0.7, metallic: 0.8, ..default() })),
            ...
            BuildingType::Shop => {
                ...
                building.spawn((
                    ...
                    MeshMaterial3d(materials.add(StandardMaterial { base_color: Color::srgb(0.38, 0.24, 0.15), perceptual_roughness: 0.85, ..default() })),
