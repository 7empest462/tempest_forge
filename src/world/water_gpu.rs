...
use bevy::render::{
    ...
    render_resource::{binding_types::{uniform_buffer, storage_buffer, storage_buffer_read_only}, *},
...
pub const WATER_GRID_SIZE: u32 = 128;
...
pub struct WaterGpuHandles {
    pub height_current: Handle<ShaderStorageBuffer>,
    pub height_next: Handle<ShaderStorageBuffer>,
    pub flow_x_current: Handle<ShaderStorageBuffer>,
    pub flow_x_next: Handle<ShaderStorageBuffer>,
    pub flow_y_current: Handle<ShaderStorageBuffer>,
    pub flow_y_next: Handle<ShaderStorageBuffer>,
    pub wall_mask: Handle<ShaderStorageBuffer>,
}
...
pub fn setup_water_gpu_buffers(
    ...
    let create_buffer = |data: Vec<u32>, shader_buffers: &mut ResMut<Assets<ShaderStorageBuffer>>| -> Handle<ShaderStorageBuffer> {
        let mut buffer = ShaderStorageBuffer::from(data);
        buffer.buffer_description.usage |= BufferUsages::COPY_SRC;
        shader_buffers.add(buffer)
    };
    
    let create_buffer_f32 = |data: Vec<f32>, shader_buffers: &mut ResMut<Assets<ShaderStorageBuffer>>| -> Handle<ShaderStorageBuffer> {
        let mut buffer = ShaderStorageBuffer::from(data);
        buffer.buffer_description.usage |= BufferUsages::COPY_SRC;
        shader_buffers.add(buffer)
    };
    ...
    commands
        ...
        .observe(|event: On<ReadbackComplete>, mut query: Query<&mut crate::world::water::WaterSimData>| {
            info!("OBSERVER FIRED!");
            ...
            if data.len() > 0 {
                ...
                info!("READBACK: len={}, max={}, min={}, changed={}", data.len(), max_height, min_height, changed);
            }
        });
...
impl Plugin for WaterComputePlugin {
    fn build(&self, app: &mut App) {
        ...
        render_app
            ...
            .add_systems(Render, prepare_water_pipeline.in_set(RenderSystems::PrepareResources))
            .add_systems(Render, queue_water_bind_group.in_set(RenderSystems::PrepareBindGroups))
...
fn swap_water_buffers(handles: Option<ResMut<WaterGpuHandles>>, query: Query<Entity, With<Readback>>) {
...
fn prepare_water_pipeline(
    ...
    if *state == WaterComputeState::Ready { return; }
    ...
    if let (CachedPipelineState::Ok(_), CachedPipelineState::Ok(_), CachedPipelineState::Ok(_)) = (&f, &o, &h) {
        info!("Water compute pipelines are READY!");
        *state = WaterComputeState::Ready;
    } else {
        info!("Water compute pipelines state: flow={:?}, out={:?}, height={:?}", f, o, h);
    }
}
...
fn queue_water_bind_group(
    ...
    let (Some(handles), Some(params)) = (gpu_handles, params) else { return };

    let get_buffer = |handle| -> Option<&Buffer> {
        shader_buffers.get(handle).map(|b| &b.buffer)
    };

    let Some(height_current) = get_buffer(&handles.height_current) else { return };
    let Some(height_next) = get_buffer(&handles.height_next) else { return };
    let Some(flow_x_current) = get_buffer(&handles.flow_x_current) else { return };
    let Some(flow_x_next) = get_buffer(&handles.flow_x_next) else { return };
    let Some(flow_y_current) = get_buffer(&handles.flow_y_current) else { return };
    let Some(flow_y_next) = get_buffer(&handles.flow_y_next) else { return };
    let Some(wall_mask) = get_buffer(&handles.wall_mask) else { return };
    ...
    let bind_group = render_device.create_bind_group(
        ...
        &[
            ...
            BindGroupEntry { binding: 1, resource: height_current.as_entire_binding() },
            BindGroupEntry { binding: 2, resource: height_next.as_entire_binding() },
            BindGroupEntry { binding: 3, resource: flow_x_current.as_entire_binding() },
            BindGroupEntry { binding: 4, resource: flow_y_current.as_entire_binding() },
            BindGroupEntry { binding: 5, resource: flow_x_next.as_entire_binding() },
            BindGroupEntry { binding: 6, resource: flow_y_next.as_entire_binding() },
            BindGroupEntry { binding: 7, resource: wall_mask.as_entire_binding() },
        ],
    );

    commands.insert_resource(WaterGpuBindGroups { bind_group, params_buffer });
}
...
fn dispatch_water_compute(
    ...
    let flow_pipeline = pipeline_cache.get_compute_pipeline(pipelines.flow_pass_id).unwrap();
    let outflow_pipeline = pipeline_cache.get_compute_pipeline(pipelines.outflow_pass_id).unwrap();
    let height_pipeline = pipeline_cache.get_compute_pipeline(pipelines.height_pass_id).unwrap();
